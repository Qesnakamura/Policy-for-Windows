...
Function Get-itunesApplication(){
    ...
    <#
    .SYNOPSIS
    This function is used to get an Windows application from the itunes store using the Apple REST API interface
    .DESCRIPTION
    ...
    Gets an Windows application from itunes store
    .EXAMPLE
    Get-itunesApplication -SearchString "Microsoft Corporation" -Limit 10
    Gets an Windows application from itunes store with a limit of 10 results
    .NOTES
...
Function Add-WindowsApplication(){
    
    <#
    .SYNOPSIS
    This function is used to add an Windows application using the Graph API REST interface
    .DESCRIPTION
    The function connects to the Graph API Interface and adds an Windows application from the itunes store
    .EXAMPLE
    Add-WindowsApplication -AuthHeader $AuthHeader
    Adds an Windows application into Intune from itunes store
    .NOTES
    NAME: Add-WindowsApplication
    #>
...
Write-Host
Write-Warning "This script adds all your Windows Store Apps as 'available' assignments for the Azure AD group you specify."
Write-Host
...
if($Applications) {
    ...
    $itunesApp = $itunesApps.results | ? { ($_.trackName).contains("$Application") }
        ...
        write-host "More than 1 application was found in the itunes store" -f Cyan
            ...
            $Create_App = Add-WindowsApplication -itunesApp $iApp
        ...
        $Create_App = Add-WindowsApplication -itunesApp $itunesApp
...
# No Applications have been specified
    ...
    write-host
    write-host "Number of Windows applications to add:" $itunesApps.results.count -f Yellow
    Write-Host
