...
.EXAMPLE
Get-ManagedAppProtection -id $id -OS "Windows"
Returns a managed app protection policy for Windows configured in Intune
...
$graphApiVersion = "Beta"
    ...
    try {
        ...
        break
            ...
            write-host "No OS parameter specified, please provide an OS. Supported values are Windows,iOS, and Windows..." -f Red
            Write-Host
            break
            ...
            $Resource = "deviceAppManagement/windowsManagedAppProtections('$id')/?`$expand=deploymentSummary,apps,assignments"
...
$AssignmentCount = 0
    ...
    foreach($ManagedAppPolicy in $ManagedAppPolicies){
        ...
        # If Windows Managed App Policy
    
        if($ManagedAppPolicy.'@odata.type' -eq "#microsoft.graph.windowsManagedAppProtection"){

            $WindowsManagedAppProtection = Get-ManagedAppProtection -id $ManagedAppPolicy.id -OS "Windows"
            
            $MAMApps = $WindowsManagedAppProtection.apps

            $WindowsAssignments = ($WindowsManagedAppProtection | select assignments).assignments
    
            if($WindowsAssignments){
    
                foreach($Group in $WindowsAssignments.target){
                    ...
                    $targetedAppManagementLevels = $WindowsManagedAppProtection.targetedAppManagementLevels
                    ...
                    write-host $WindowsManagedAppProtection.displayname -ForegroundColor Green
                    ...
                    Write-Host "Jailbroken/rooted devices blocked: $($WindowsManagedAppProtection.deviceComplianceRequired)"
                    Write-Host "Min OS version: $($WindowsManagedAppProtection.minimumRequiredOsVersion)"
                    Write-Host "Min patch version: $($WindowsManagedAppProtection.minimumRequiredPatchVersion)"
                    Write-Host "Allowed device manufacturer(s): $($WindowsManagedAppProtection.allowedWindowsDeviceManufacturers)"
                    write-host "Require managed browser: $($WindowsManagedAppProtection.managedBrowserToOpenLinksRequired)"
                    Write-Host "Contact sync blocked: $($WindowsManagedAppProtection.contactSyncBlocked)"
                    Write-Host "Printing blocked: $($WindowsManagedAppProtection.printblocked)"
                    Write-Host
                    write-host "-------------------------------------------------------------------"
                    write-host
        ...
        if($ManagedAppConfiguration.assignments){
            ...
            foreach($group in $ManagedAppConfiguration.assignments){
                ...
                foreach($MAMApp in $MAMApps){
                    ...
                    if($MAMApp.mobileAppIdentifier.'@odata.type' -eq "#microsoft.graph.windowsMobileAppIdentifier"){
