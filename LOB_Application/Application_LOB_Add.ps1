...
function GetWindowsAppBody($displayName, $publisher, $description, $filename, $identityName, $identityVersion, $versionName, $minimumSupportedOperatingSystem){

	$body = @{ "@odata.type" = "#microsoft.graph.windowsLOBApp" };
	$body.categories = @();
...
<#
.SYNOPSIS
This function is used to get information about an Windows APK file using the Windows SDK - https://developer.windows.com/studio/index.html
.DESCRIPTION
This function is used to get information about an Windows APK file using the Windows SDK - https://developer.windows.com/studio/index.html
.EXAMPLE
...
(
    ...
    Write-Host
    Write-Host "Windows SDK isn't installed..." -ForegroundColor Red
    Write-Host "Please install Windows Studio and install the SDK from https://developer.windows.com/studio/index.html"
    Write-Host
    break

    }

    if(((gci $WindowsSDK | select name).Name).count -gt 1){

    $BuildTools = ((gci $WindowsSDK | select name).Name | sort -Descending)[0]

    }

    else {

    $BuildTools = ((gci $WindowsSDK | select name).Name)

    }

$aaptPath = "$WindowsSDK\$BuildTools"
...
$WindowsPackage = $aaptRun | ? { ($_).startswith("package") }

$PackageInfo = $WindowsPackage.split(" ")
...
function Upload-WindowsLob(){

<#
.SYNOPSIS
This function is used to upload an Windows LOB Application to the Intune Service
.DESCRIPTION
This function is used to upload an Windows LOB Application to the Intune Service
.EXAMPLE
Upload-WindowsLob -sourceFile "C:\Software\package.apk" -publisher "Publisher Name" -description "Description of Application" -identityName "com.package" -identityVersion "1" -versionName "10.1.1"
This example uses all parameters required to add an Windows Application into the Intune Service
Upload-WindowsLob -sourceFile "C:\Software\package.apk" -publisher "Publisher Name" -description "Description of Application"
This example uses the required parameters to add an Windows Application into the Intune Service. This example will require the Windows SDK to get identityName and identityVersion
.NOTES
NAME: Upload-WindowsLOB
#>
...
    [ValidateNotNullOrEmpty()]
	...
	try
	{
        ...
        $LOBType = "microsoft.graph.windowsLOBApp"
        ...
        Test-SourceFile "$SourceFile"
            ...
            $APKInformation = Get-ApkInformation -WindowsSDK $WindowsSDKLocation -sourceFile "$SourceFile"
        ...
        # Create a new Windows LOB app.
        Write-Host
        Write-Host "Creating JSON data to pass to the service..." -ForegroundColor Yellow
		$mobileAppBody = GetWindowsAppBody "$displayName" "$Publisher" "$Description" "$filename" "$identityName" "$identityVersion" "$versionName";
        ...
        [xml]$manifestXML = '<?xml version="1.0" encoding="utf-8"?><WindowsManifestProperties xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><Package>com.leadapps.windows.radio.ncp</Package><PackageVersionCode>10</PackageVersionCode><PackageVersionName>1.0.5.4</PackageVersionName><ApplicationName>A_Online_Radio_1.0.5.4.apk</ApplicationName><MinSdkVersion>3</MinSdkVersion><AWTVersion></AWTVersion></WindowsManifestProperties>'

        $manifestXML.WindowsManifestProperties.Package = "$identityName" # com.application.test
        $manifestXML.WindowsManifestProperties.PackageVersionCode = "$identityVersion" # 10
        $manifestXML.WindowsManifestProperties.PackageVersionName = "$identityVersion" # 1.0.5.4
        $manifestXML.WindowsManifestProperties.ApplicationName = "$filename" # name.apk
...
# Path to Windows SDK Location to find aapt.exe tool if you don't specify identityName or identityVersion
...
$WindowsSDKLocation = "C:\WindowsSDK\build-tools"
...
#### Without Windows SDK - All parameters are specified
# Upload-WindowsLob -sourceFile "C:\Software\OnlineRadio\A_Online_Radio_1.0.5.4.apk" -publisher "A Online Radio" -description "A Online Radio 1.0.5.4" -identityName "com.leadapps.windows.radio.ncp" -identityVersion "10" -versionName "1.0.5.4"

#### With Windows SDK:
# Upload-WindowsLob -sourceFile "C:\Software\OnlineRadio\A_Online_Radio_1.0.5.4.apk" -publisher "A Online Radio" -description "A Online Radio 1.0.5.4"
