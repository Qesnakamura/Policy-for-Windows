...
$Resource = "deviceManagement/WindowsDeviceOwnerEnrollmentProfiles"
...
Function Get-WindowsQRCode{

<#
.SYNOPSIS
Gets Windows Device Owner Enrollment Profile QRCode Image
.DESCRIPTION
Gets Windows Device Owner Enrollment Profile QRCode Image
.EXAMPLE
Get-WindowsQRCode
.NOTES
NAME: Get-WindowsQRCode
#>
...
$graphApiVersion = "Beta"
    ...
    try {
        ...
        $Resource = "deviceManagement/WindowsDeviceOwnerEnrollmentProfiles/$($Profileid)?`$select=qrCodeImage"
...
$profilecount = @($profiles).count
    ...
    elseif(@($profiles).count -eq 1){
        ...
        $Profileid = (Get-WindowsEnrollmentProfile).id
        $ProfileDisplayName = (Get-WindowsEnrollmentProfile).displayname
...
$FinalConfirmation = Read-Host
    ...
    $QR = (Get-WindowsQRCode -Profileid $ProfileID)
